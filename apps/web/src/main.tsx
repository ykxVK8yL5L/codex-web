...
function App() {
  ...
  return (
    ...
    <div className={`app ${page === "sessions" ? "task-layout" : "wide-layout"}`}>
      ...
      {page === "previews" && <PreviewsPage sessionToken={sessionToken} projects={projects} sessions={sessions} t={t} notify={notify} onOpenMainNav={() => setMainNavOpen(true)} />}
      {page === "projects" && <ProjectsPage sessionToken={sessionToken} projects={projects} sessions={sessions} notify={notify} onOpenSession={navigateSession} onNewProjectSession={(projectId) => {
...
function SessionPage({
  ...
  const [mobileContextPanel, setMobileContextPanel] = useState<"changes" | "activity" | null>(null);
  ...
  async function openTaskLog() {
    ...
    setTaskLogPanel({ log: result.log || t("session.noTaskLog") });
  }
          ...
          <div className="header-title-row">
            ...
            <div>
              <div className="session-title-line">
                ...
                <h1>{pageSessionTitle ?? t("session.untitled")}</h1>
              </div>
        ...
        <div className="dialog-layer mobile-context-layer" role="presentation">
          ...
          <section className="dialog-card mobile-context-card" role="dialog" aria-modal="true">
            <div className="dialog-head">
              <div>
                <strong>{mobileContextPanel === "activity" ? t("session.activityTitle") : t("workspace.changes")}</strong>
...
function ContextPanel({
  ...
  initialPanel?: "changes" | "activity";
  modal?: boolean;
}) {
  const dialog = useAppDialog(t);
  const [activePanel, setActivePanel] = useState<"changes" | "activity">(initialPanel);
  ...
  useEffect(() => {
    ...
    setActivityCursor(null);
    setActivityHasMore(false);
    if (activePanel === "activity") void loadActivity();
  }, [activePanel, session?.id, sessionToken]);
  ...
  }, [activePanel, scheduleLoadActivity, session?.id]);
  ...
  return (
    <aside className={`context-panel ${modal ? "mobile-context-panel" : ""}`}>
      ...
      <header className="context-header">
        ...
        <button className="ghost-button icon-only" type="button" title={t("action.refresh")} aria-label={t("action.refresh")} onClick={() => activePanel === "changes" ? void loadChanges() : void loadActivity()} disabled={!session}><IconText icon={RefreshCw}>{t("action.refresh")}</IconText></button>
      </header>
...
function AutomationsPage({
  ...
  async function createAutomation(event: React.FormEvent) {
    event.preventDefault();
    if (!/^(manual|hourly|daily\s+[0-2]\d:[0-5]\d)$/i.test(schedule.trim())) {
  ...
  async function editAutomation(automation: AutomationSummary) {
    const nextPrompt = await dialog.prompt({
      title: t("automation.editPrompt"),
      message: automation.name,
      defaultValue: automation.prompt,
      placeholder: t("form.automationPrompt"),
      confirmLabel: t("action.save"),
      multiline: true,
    });
    if (nextPrompt === null) return;
    await updateAutomation(automation, { prompt: nextPrompt });
  }
  ...
  async function runAutomation(automation: AutomationSummary) {
    ...
    const session = (await response.json()) as SessionSummary;
    await onChange();
    notify(t("automation.runStarted"), "success");
    onOpenSession(session.id);
  }
    ...
    <main className="management-page automation-page">
      ...
      <section className="management-layout">
        <form className="management-form" onSubmit={createAutomation}>
          ...
          <input name="schedule" value={schedule} onChange={(event) => setSchedule(event.target.value)} placeholder={t("form.automationSchedule")} required />
            ...
            {automationItems.map((automation) => (
              <article className="project-list-card" key={automation.id}>
                <div className="project-list-title">
                  ...
                  <span className={`pill ${automation.status === "paused" ? "warm" : ""}`}>{automation.status}</span>
                </div>
                <code>{automation.prompt}</code>
                <div className="project-list-meta">
                ...
                <div className="project-list-actions">
                  ...
                  <Button className="icon-only" variant="outline" size="sm" type="button" title={t("automation.editPrompt")} aria-label={t("automation.editPrompt")} onClick={() => void editAutomation(automation)}><IconText icon={Pencil}>{t("automation.editPrompt")}</IconText></Button>
        ...
        <div className="workspace-modal compact-modal" role="dialog" aria-modal="true">
          ...
          <div className="extension-detail">
            ...
            {runsPanel.runs?.map((run) => (
              <button className="file-list-item" key={run.id} type="button" onClick={() => onOpenSession(run.sessionId)}>
                <span>{run.status} · exit {run.exitCode ?? "null"}</span>
      ...
      {createPanelOpen && (
        <div className="workspace-modal compact-modal automation-create-modal" role="dialog" aria-modal="true">
          ...
          <form className="management-form" onSubmit={createAutomation}>
            ...
            <input name="mobile-schedule" value={schedule} onChange={(event) => setSchedule(event.target.value)} placeholder={t("form.automationSchedule")} required />
